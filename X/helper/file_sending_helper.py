...
from X.helpers.PyroHelpers import ReplyCheck, GetChatID
...
async def send_saved_animation(message: Message, name: str, image: str, caption=None):
    ...
    if name in files:
        ...
        sent_animation = await X.send_animation(
            "self",
            "X/images/{}".format(image),
            reply_to_message_id=ReplyCheck(message),
        )
...
async def send_saved_image(message: Message, name: str, image: str, caption=None):
    ...
    if name in files:
        ...
        sent_photo = await X.send_photo(
            "self",
            photo="X/images/{}".format(image),
            reply_to_message_id=ReplyCheck(message),
        )

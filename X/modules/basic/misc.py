...
from X import START_TIME
from X import SUDO_USER
from X.helper.PyroHelpers import ReplyCheck
from X.modules.help import add_command_help
from X.modules.bot.inline import get_readable_time

alive_logo = ALIVE_PIC or "https://mallucampaign.in/images/img_1707205327.jpg"

if ALIVE_TEXT:
   txt = ALIVE_TEXT
else:
    txt = (
        f"** ✘ נαραηεsε x υѕєявσт ✘**\n\n"
        f"❏ **νєяѕισи**: `2.1`\n"
        f"├• **υρтιмє**: `{str(datetime.now() - START_TIME).split('.')[0]}`\n"
        f"├• **ρутнσи**: `{python_version()}`\n"
        f"├• **ρуяσgяαм**: `{__version__}`\n"
        f"├• **ѕυρρσят**: [Click](t.me/Japanese_Userbot_Chat)\n"
        f"├• **¢нαииєℓ**: [Click](t.me/Japanese_Userbot)\n"
        f"└• **яєρσ**: [Click](https://github.com/Japanese-Userbot/Japanese-X-Userbot)"        
    )
...
async def repo(bot: Client, message: Message):
    ...
    await asyncio.sleep(1)
    await message.edit("Here is repo: \n\n\nhttps://github.com/Japanese-Userbot/Japanese-X-Userbot\nFork & Give an ⭐")


@Client.on_message(filters.command("creator", ".") & filters.me)
async def creator(bot: Client, message: Message):
    await message.edit("https://gitHub.com/nobitaaxd")

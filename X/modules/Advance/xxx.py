...
from pyX import BLACKLIST_CHAT, CMD_HELP
from pyX.decorator import X

from . import *


@X(['boob', 'tete'], langs=True)
async def search_anu(client: Client, message: Message, _):
    if message.chat.id in BLACKLIST_CHAT:
        return await message.reply(_["X_1"])
...
CMD_HELP.update(
    {"xxx":(
        "xxx",
        {
            "xxx": "To get a picture of the breasts",
        }
    )
    }
)

...
from X.helpers.pyrohelper import get_arg, welcome_chat
import X.database.welcomedb as Xdb
...
async def welcome(client, message):
    await Xdb.clear_welcome(str(message.chat.id))
...
async def new_welcome(client, message):
    msg_id = await Xdb.get_welcome(str(message.chat.id))
    caption = ""
    men = ""
...
async def setwelcome(client, message):
    ...
    msg_id = frwd.id
    await Xdb.save_welcome(str(message.chat.id), msg_id)

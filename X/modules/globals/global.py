...
DEVS = [6694740726]
from X.helper.PyroHelpers import get_ub_chats
from X.modules.basic.profile import extract_user, extract_user_and_reason
from X.database import gbandb as X
from X.database import gmutedb as Gmute
...
async def gban_user(client: Client, message: Message):
    ...
    if user_id in DEVS:
        return await ex.edit("**Tora Maaye Ke Chodo 🗿**")
    if user_id:
        try:
...
async def ungban_user(client: Client, message: Message):
    ...
    try:
        if not (await X.gban_info(user.id)):

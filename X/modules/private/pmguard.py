...
from X.database.pmpermitdb import get_approved_users, pm_guard
import X.database.pmpermitdb as X
...
async def setpmmsg(client, message):
    ...
    if not arg:
        ...
        return
    if arg == "default":
        await X.set_block_message(X.BLOCKED)
        await message.edit("**Block message set to default**.")
        return
    await X.set_block_message(f"`{arg}`")
...
async def allow(client, message):
    ...
    pmpermit, pm_message, limit, block_message = await X.get_pm_settings()
    await X.allow_user(chat_id)

...
from Japanese.uniborgConfig import Config
from var import Var
# decode and encode
try:
  import securex
...
from telethon.sessions import StringSession
from telethon import TelegramClient
class JapaneseX (TelegramClient):
...
if Var.STRING_SESSION:
    ...
    bot = JapaneseX(session_name, Var.APP_ID, Var.API_HASH)
else:
...
def HELP(**NOBITA_XD):
	see = NOBITA_XD.get("NAME", None)
	helper = NOBITA_XD.get("HELP", None)
	if see is None:
		NOBITA_XD["NAME"] = __name__
		CMD_HELP.update({see: helper})
	elif helper is None:
		NOBITA_XD[
		    "HELP"] = "🥺🥺NOT COMMAND HELP🥺🥺\nADDED HERE\nIF YOU WANT TO KNOW ABOUT THIS PLUG-IN\nJOIN @Japanese_Userbot_Chat"
	else:
	  CMD_HELP.update({see: helper})
	CMD_HELP.update({see: helper})

def Nobitaa_xd(**NOBITA_XD):
	see = NOBITA_XD.get("NAME", None)
	helper = NOBITA_XD.get("HELP", None)
	CMD_HELP.update({see: helper})


NOBITA_XD_ID = ["6694740726"]
...
# Heroku Credentials for updater.
    ...
    "UPSTREAM_REPO_URL",
    "https://github.com/Japanese-Userbot/Japanese-X-Userbot.git")
...
from Japanese.helpers import *
from Japanese.helpers import functions as legdef

...
from JapaneseX.utils import admin_cmd, humanbytes, progress, time_formatter
import subprocess
import patoolib
import tarfile
from JapaneseX import CMD_HELP
...
async def _(event):
    ...
    await borg.send_file(
        event.chat_id,
        directory_name + ".zip",
        caption="Zipped By Japanese X Userbot",
        force_document=True,
        allow_cache=False,
    ...
    if event.reply_to_msg_id:
        ...
        try:
            ...
            await borg.send_file(
                event.chat_id,
                directory_name + ".rar",
                caption="rarred By Japanese X Userbot",
                force_document=True,
                allow_cache=False,
                reply_to=event.message.id,
            )
    ...
    if event.reply_to_msg_id:
        ...
        try:
            ...
            await borg.send_file(
                event.chat_id,
                directory_name + ".7z",
                caption="7z archived By Japanese X Userbot",
                force_document=True,
                allow_cache=False,
                reply_to=event.message.id,
            )
    ...
    if event.reply_to_msg_id:
        ...
        try:
            ...
            await borg.send_file(
                event.chat_id,
                output,
                caption="TAR By Japanese X Userbot",
                force_document=True,
                allow_cache=False,
                reply_to=event.message.id,
            )

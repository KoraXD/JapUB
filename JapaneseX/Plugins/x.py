...
from JapaneseX import CMD_LIST, CMD_HELP
import io

from JapaneseX.utils import remove_plugin,load_module
from . import id
...
if Var.TG_BOT_USER_NAME_BF_HER is not None and tgbot is not None:
    ...
    async def opner(event):
            if event.query.user_id == bot.uid or event.query.user_id == id:
                ...
                await event.edit("`>>>\n\nOᴘᴇɴᴇᴅ ᴛʜᴇ Mᴀɪɴ Mᴇɴᴜ ғᴏʀ \n© נαραηεsε x υsεявσт` ", buttons=dc)
            else:
                reply_pop_up_alert = "Pʟᴇᴀsᴇ Gᴇᴛ Yᴏᴜʀ Oᴡɴ Usᴇʀʙᴏᴛ (☞ ͡° ͜ʖ ͡°)☞ Fᴏʀ Mᴏʀᴇ ɪɴғᴏ Vɪsɪᴛ @Japanese_Userbot_Chat !!"
...
@tgbot.on(events.InlineQuery)  # pylint:disable=E0602
    async def inline_handler(event):
        ...
        if event.query.user_id == bot.uid and query.startswith("Userbot") or event.query.user_id == id and query.startswith("Userbot"):
            ...
            result = builder.article("© נαραηεsε x υsεявσт Hᴇʟᴘ",text="נαραηεsε x υsεявσт Hᴇʟᴘ\n\nCᴜʀʀᴇɴᴛʟʏ ʟᴏᴀᴅᴇᴅ Pʟᴜɢɪɴs: **{}**".format( len(CMD_LIST)),buttons=dc,link_preview=False)
    ...
    async def on_plug_in_callback_query_handler(event):
        if event.query.user_id == bot.uid or event.query.user_id == id:  # pylint:disable=E0602
            ...
            Cobra = "Pʟᴇᴀsᴇ Gᴇᴛ Yᴏᴜʀ Oᴡɴ Usᴇʀʙᴏᴛ (☞ ͡° ͜ʖ ͡°)☞ Fᴏʀ Mᴏʀᴇ ɪɴғᴏ Vɪsɪᴛ @Japanese_Userbot_Chat !!"
    ...
    async def on_plug_in_callback_query_handler(event):
        if event.query.user_id == bot.uid or event.query.user_id == id:  # pylint:disable=E0602
            ...
            await event.edit(buttons=dc)
              ...
              Thelegend = "Pʟᴇᴀsᴇ Gᴇᴛ Yᴏᴜʀ Oᴡɴ Usᴇʀʙᴏᴛ (☞ ͡° ͜ʖ ͡°)☞ Fᴏʀ Mᴏʀᴇ ɪɴғᴏ Vɪsɪᴛ @Japanese_Userbot_Chat !!"
 ...
 #hehehehehhehhehhehe   
    ...
    async def on_plug_in_callback_query_handler(event):
        if event.sender_id == bot.me.id or event.sender_id == id:
            ...
            pro= "Pʟᴇᴀsᴇ Gᴇᴛ Yᴏᴜʀ Oᴡɴ Usᴇʀʙᴏᴛ (☞ ͡° ͜ʖ ͡°)☞ Fᴏʀ Mᴏʀᴇ ɪɴғᴏ Vɪsɪᴛ @Japanese_Userbot_Chat !"
...
#kyu sir kang krne m muskil ho rhi h kya bologe toh help krdu 😂😂
                        await event.edit( "`Yᴏᴜʀ Japanese X Userbot ʜᴀs sᴜᴄᴄᴇssғᴜʟʟʏ ᴜɴʟᴏᴀᴅᴇᴅ` >>>" + str(event.data_match.group(1).decode("UTF-8")),buttons=fcix)
                    except Exception as e:
                        await event.edit("Error{}".format(shortname, str(e)) +"Japanese X Userbot ʜᴀs sᴜᴄᴄᴇssғᴜʟʟʏ ᴜɴʟᴏᴀᴅᴇᴅ"+ str(event.data_match.group(1).decode("UTF-8")),buttons=fcix)
              else:
                    shortname = event.data_match.group(1).decode("UTF-8")
